...
public class tk2dUIScrollView : MonoBehaviour 
{
    ...
    [SerializeField] tk2dUIItem backgroundBtn;
	...
	bool isDown, resetChild, isInProgress, isSnap;

	float swipeVelocity = 0f;
	float snapToLimitsVelocity = 0f;

	float startTouchValue = 0f;
	float previousTouchWorldValue = 0f;
	float cachedTouchWorldValue = 0f;
	float beginContentOffsetValue = 0f;
    float downOffsetValue = 0f;

	float fromSnapValue, toSnapValue;
	float startSnapVelocity, currentSnapVelocity;
	Transform snapItem = null;
	...
	void Down(tk2dUIItem btn) 
    {
        if (contentLength > float.Epsilon)
        {
            tk2dBaseMesh.globalDisableRoundShift = true;
            ...
            swipeVelocity = 0f;
            snapToLimitsVelocity = 0f;
            isSnap = resetChild = isInProgress = false;
            isDown = true;
        }
	}

	void Up(tk2dUIItem btn)
    {
        if (!isDown)
        {
            return;
        ...
        if (Abs(swipeDistance) > SwipeThreshold())
        {
            ...
            {
                tk2dBaseMesh.globalDisableRoundShift = false;
            }
        }
        else
        {
	...
	bool OutsideLimits(float min, float max, float value) 
    {
	...
	float CalculateDestinationValue(float velocity, float min, float max) 
    {
		float destination = ContentOffsetValue;
