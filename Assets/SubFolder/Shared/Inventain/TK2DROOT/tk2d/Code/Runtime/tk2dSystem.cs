...
#region platforms
	...
	static string currentPlatform = "2x"; // Not serialized, this should be set up on wake
	const string defaultPlatform = "2x";

    public static bool IsRetina
    {
        get
        {
            return tk2dSystem.CurrentPlatform.Equals("4x");
        }
    }
	...
	public static string CurrentPlatform
	{
		get 
		{
			if (!currentPlatformInitialized)
			{
				...
				currentPlatform = UnityEditor.EditorPrefs.GetString("tk2d_platform", defaultPlatform);
#else
				currentPlatform = (tk2dSystem.inst_NoCreate != null) ? tk2dSystem.inst_NoCreate.usePlatform : defaultPlatform;

                if (Screen.height > iPadRetinaGraphicsTreshhold || Screen.width > iPadRetinaGraphicsTreshhold)
                {
                    currentPlatform = "4x";
                }
                else
...
#region Resources
	...
	[SerializeField]
	string usePlatform = "2x";

	#if !UNITY_EDITOR
    [SerializeField] static int iPadRetinaGraphicsTreshhold = 1900;
	#endif
	...
	#if UNITY_EDITOR
	public void SetUsePlatform(string value) { usePlatform = value; } 
	public string UsePlatform{ get { return usePlatform; } }
