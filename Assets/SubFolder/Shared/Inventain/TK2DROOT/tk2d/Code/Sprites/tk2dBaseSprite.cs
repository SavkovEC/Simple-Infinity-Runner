...
/// </summary>
public abstract class tk2dBaseSprite : tk2dBaseMesh, tk2dRuntime.ISpriteCollectionForceBuild
{
	/// <summary>
	/// Anchor.
    ...
    public bool isLayoutSizeLocked;
	...
	/// <summary>
	///  use this to round vertices
	/// </summary>
	protected Vector3 FloorVertex(Vector3 vertex)
	{
		return vertex.FloorVector() - TotalShift;
	}


	/// <summary>
	/// this is not suitable for "general rounding" used in our tk2d
	/// it can be used where rounding occures implicitly before vertex calculations
	/// for example in progress bar - it rounds mFillAmount itself to achive rounding in both vertices and uv
	/// and after this just corrects resulted vertices using this funciton
	/// </summary>
	protected Vector3 RoundVertex(Vector3 vertex)
	{
		return vertex.RoundVector() - TotalShift;
	}
	...
	public static T AddComponent<T>(GameObject go, tk2dSpriteCollectionData spriteCollection, string spriteName) where T : tk2dBaseSprite
	{
		...
		if (spriteId == -1) {
			CustomDebug.LogError( string.Format("Unable to find sprite named {0} in sprite collection {1}", spriteName, spriteCollection.spriteCollectionName) );
			return null;
		}
		else {
	...
	{
		return CurrentSprite.positions.Length;
	}
	
	protected int GetNumIndices()
	{
		return CurrentSprite.indices.Length;
	}
	...
	protected void SetPositions(Vector3[] positions, Vector3[] normals, Vector4[] tangents)	
	{
		...
		for (int i = 0; i < numVertices; ++i)
		{
			...
			{
				positions[i] = FloorVertex(positions[i]);
			}
		}
	...
	public Bounds GetBounds()
	{
		...
		return new Bounds(new Vector3(sprite.boundsData[0].x * _scale.x, sprite.boundsData[0].y * _scale.y, sprite.boundsData[0].z * _scale.z),
            new Vector3(sprite.boundsData[1].x * Mathf.Abs(_scale.x) - sprite.innerPadding, sprite.boundsData[1].y * Mathf.Abs(_scale.y) - sprite.innerPadding, sprite.boundsData[1].z * Mathf.Abs(_scale.z) ));
	}
	...
	protected virtual void CreateCollider()
	{
		...
		if (sprite.physicsEngine == tk2dSpriteDefinition.PhysicsEngine.Physics3D) {
			...
			else if (sprite.colliderType != tk2dSpriteDefinition.ColliderType.None)
			{
				...
				{
					CustomDebug.LogError("Invalid mesh collider on sprite '" + name + "', please remove and try again.");
				}
			}
