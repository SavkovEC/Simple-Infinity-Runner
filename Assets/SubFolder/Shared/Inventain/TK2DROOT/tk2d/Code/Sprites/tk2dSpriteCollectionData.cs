		...
		get 
		{
			if (platformSpecificData == null)
			{
				if (hasPlatformData)
				{
                    ...
                    if (guid.Length == 0)
                    {
						CustomDebug.Log("failed to find platform by guid!");
			...
			else 
            {
                ...
                if (streamingTextures.Length > 0) 
                {
                    ...
                    for (int i = 0; i < streamingTextures.Length; ++i)
                    {
                        ...
                        path = PathUtils.GetStreamingAssetsPath() + "/" + path;

                        Texture2D tex = TextureCache.LoadTexture(path, TextureDataSource.StreamingAssets, TextureFormat.RGBA32, textureMipMaps);
                        tex.name = string.Format("{0}PNG_{1}_{2}", internalResourcePrefix, name, i);
...
//                        tex.Apply(textureMipMaps, true);
    ...
    {
        bool stream = streamingTextures.Length > 0;
    ...
    void DestroyMaterialInsts()
    {
        if (materialInsts != null)
        {
            foreach (Material mat in materialInsts) 
            {
    ...
    int retainCount;
    HashSet<object> renderRefs = new HashSet<object>();
    ...
    int RetainCount 
    {
        ...
        set 
        {
            ...
            {
//                Debug.Log("load : " + this.spriteCollectionName);
            }

            if(retainCount > 0 && value == 0)
            {
//                Debug.Log("unload : " + this.spriteCollectionName);
//                this.ResetPlatformData();
    ...
    public static tk2dSpriteCollectionData operator + (tk2dSpriteCollectionData platform, object sprite) 
    {
        if (platform != null)
        {
            platform.Add(sprite);
