...
public class tk2dCamera : MonoBehaviour 
{
	...
	private Camera UnityCamera {
		get {
			if (_unityCamera == null) {
				...
				if (_unityCamera == null) {
					CustomDebug.LogError("A unity camera must be attached to the tk2dCamera script");
				}
			}
			return _unityCamera;
		}
	...
	private void Upgrade() {
		if (version != CURRENT_VERSION) {
			...
			CustomDebug.Log("tk2dCamera '" + this.name + "' - Upgraded from version " + version.ToString());
	...
	public void UpdateCameraMatrix()
	{
		...
		if (inheritedCameraSettings.projection == tk2dCameraSettings.ProjectionType.Perspective) {
            ...
            if (!m.Equals(unityCamera.projectionMatrix))
            {
