...
public class tk2dSpriteAnimationEditorPopup : EditorWindow 
{
	...
	tk2dSpriteAnimationClip selectedClip
	{
		...
		set 
		{
			...
			if (selectedClipId == -1)
			{
				if (value != null) CustomDebug.LogError("Unable to find clip");
				_selectedClip = null;
			}
		}
	}
	...
	void FilterClips()
	{
		List<tk2dSpriteAnimationClip> tempList = new List<tk2dSpriteAnimationClip>();
		...
		else 
		{
			foreach (tk2dSpriteAnimationClip clip in allClips)
			{
				if (!clip.Empty && Contains(clip.name, searchFilter))
				{
					tempList.Add(clip);
				}
			}
		...
		tempList.Sort(new tk2dEditor.Shared.Tk2dSpriteAnimationClipNaturalComparer());

		filteredClips = tempList;
	}
