...
public class tk2dSpriteCollectionBuilder
{
    ...
    static void PadTexture(Texture2D tex, int pad, int innerPad, tk2dSpriteCollectionDefinition.Pad padMode)
	{
			...
			{		
                switch (padMode)
                {
                    case tk2dSpriteCollectionDefinition.Pad.Extend:
                        c0 = tex.GetPixel(x, pad + innerPad);
                        c1 = tex.GetPixel(x, tex.height - 1 - pad - innerPad);
                        break;
                    case tk2dSpriteCollectionDefinition.Pad.TileX:
                    case tk2dSpriteCollectionDefinition.Pad.TileXY:
                        c1 = tex.GetPixel(x, pad + innerPad);
                        c0 = tex.GetPixel(x, tex.height - 1 - pad - innerPad);
                        break;
                }

                if (innerPad > 0 && y - pad >= 0)
                {
                    tex.SetPixel(x, y, new Color(c0.r, c0.g, c0.b, 0));
                    tex.SetPixel(x, tex.height - 1 - y, new Color(c1.r, c1.g, c1.b, 0)); // set clear color for inner padding
                }
                else
                {
                    tex.SetPixel(x, y, new Color(c0.r, c0.g, c0.b, innerPad > 0 ? 0 : c0.a));
                    tex.SetPixel(x, tex.height - 1 - y, new Color(c1.r, c1.g, c1.b, innerPad > 0 ? 0 : c1.a));
                }
			}
		}
        for (int x = 0; x < pad + innerPad; ++x)
		{
            for (int y = 0; y < tex.height + innerPad; ++y)
			{
                switch (padMode)
                {
                    case tk2dSpriteCollectionDefinition.Pad.Extend:
                        c0 = tex.GetPixel(pad + innerPad, y);
                        c1 = tex.GetPixel(tex.width - 1 - pad - innerPad, y);
                        break;
                    case tk2dSpriteCollectionDefinition.Pad.TileY:
                    case tk2dSpriteCollectionDefinition.Pad.TileXY:
                        c1 = tex.GetPixel(pad, y + innerPad);
                        c0 = tex.GetPixel(tex.width - 1 - pad - innerPad, y);
                        break;
                }

                if (innerPad > 0 && x - pad >= 0)
                {
                    tex.SetPixel(x, y, new Color(c0.r, c0.g, c0.b, 0));
                    tex.SetPixel(tex.width - 1 - x, y, new Color(c1.r, c1.g, c1.b, 0));
                }
                else
                {
                    tex.SetPixel(x, y, new Color(c0.r, c0.g, c0.b, innerPad > 0 ? 0 : c0.a));
                    tex.SetPixel(tex.width - 1 - x, y, new Color(c1.r, c1.g, c1.b, innerPad > 0 ? 0 : c1.a));
                }
			}
		}
	}
    ...
    static void DeleteUnusedAssets( List<string> oldAssets, string[] newAssets )
    {
        foreach (string asset in oldAssets)
        {
            bool found = false;
            foreach (string t in newAssets)
            {
                if (t == asset)
                {
                    found = true;
    ...
    static Texture2D ProcessTexture(tk2dSpriteCollection settings, int texIndex, bool additive, tk2dSpriteCollectionDefinition.Pad padMode, bool disableTrimming, bool isInjectedTexture, bool isDiced, Texture2D srcTex, int sx, int sy, int tw, int th, ref SpriteLut spriteLut, int padAmount)
	{
		...
		if (numNotTransparent > 0)
		{
            ...
            if (settings.textureParams.Length > texIndex)
            {
                innerPadding = settings.textureParams[texIndex].disableInnerPadding ? 0 : ((int)(settings.innerPadAmount / settings.globalScale));
            }



            Texture2D dtex = new Texture2D(w1 + (padAmount + innerPadding) * 2 , h1 + (padAmount + innerPadding) * 2);
			...
			for (int x = 0; x < w1; ++x)
			{
				for (int y = 0; y < h1; ++y)
				{
                    ...
                    dtex.SetPixel(x + padAmount + innerPadding, y + padAmount + innerPadding, col);
				}
			}
            ...
            if (settings.premultipliedAlpha)
			{
				for (int x = 0; x < dtex.width; ++x)
				{
					for (int y = 0; y < dtex.height; ++y)
					{
                        ...
                        if (settings.atlasFormat != tk2dSpriteCollection.AtlasFormat.StreamingPNG)
                        {
                            col.r *= col.a;
                            col.g *= col.a;
                            col.b *= col.a;
			...
			if (!isDiced) {
                PadTexture(dtex, padAmount, innerPadding, padMode);
			}
	...
	static bool CheckSourceAssets(tk2dSpriteCollection gen)
	{
		...
		if (missingTextures.Length > 0) {
			CustomDebug.LogError(string.Format("Error in sprite collection '{0}'\n{1}", gen.name, missingTextures));
		}
    ...
    static void PNGPostProcess(tk2dSpriteCollection gen, string texturePath)
    {
        string dataPath = Application.dataPath;

        System.Action pngCrushPostProcessAction = delegate
        {
//            Debug.Log("Starting PNGCrush post process for: " + texturePath);
//            string[] pngCrushPaths = System.IO.Directory.GetFiles(dataPath, "*pngcrush*", SearchOption.AllDirectories);
//            if (pngCrushPaths.Length > 0)
//            {
//                System.Diagnostics.Process p = new System.Diagnostics.Process();
//                p.StartInfo.Arguments = "-ow " + "'" + texturePath + "'";
//                p.StartInfo.FileName = pngCrushPaths[0];
...
//
//                p.Exited += delegate {
//                    Debug.Log("PNG Post process finished for: " + texturePath);
//                };
//            }
//            else
//            {
//                Debug.LogError("pngcrush executable not found!");
//            }
        };

        string[] paths;
        //USE POSTERIZE ALGORITHM
        if (gen.usePosterize)
        {
            Debug.Log("Start Posterize for: " + texturePath);
            paths = System.IO.Directory.GetFiles(dataPath, "*posterize.py*", SearchOption.AllDirectories);
            if (paths.Length > 0)
            {
                System.Diagnostics.Process p = new System.Diagnostics.Process();
                p.StartInfo.Arguments = "'" + paths[0] + "' '" + texturePath + "' " + gen.posterizeValue;
                ...
                p.Exited += delegate
                {
                    pngCrushPostProcessAction();
                };
            }
            else
            {
                Debug.LogError("Posterize script not found!");
                pngCrushPostProcessAction();
            }
        }
        else
    ...
    public static bool Rebuild(tk2dSpriteCollection gen)
    {
		...
		if (gen.HasPlatformData)
		{
			...
			for (int i = 0; i < gen.platforms.Count; ++i)
			{
				...
				if (validPlatform) 
				{
                    ...
                    CustomDebug.Log("base scale: " + baseScale);
                    CustomDebug.Log("this scale: " + thisScale);
            ...
            gen.spriteCollection.streamingTextures = new string[0];
		...
		for (int i = 0; i < gen.textureParams.Length; ++i)
		{
			...
			if (gen.textureParams[i].dice)
			{
				...
				for (int sx = 0; sx < srcTex.width; sx += diceUnitX)
				{
					for (int sy = 0; sy < srcTex.height; sy += diceUnitY)
					{
                        ...
                        Texture2D dest = ProcessTexture(gen, i, gen.textureParams[i].additive, tk2dSpriteCollectionDefinition.Pad.Extend, gen.textureParams[i].disableTrimming, false, true, srcTex, sx, sy, tw, th, ref diceLut, GetPadAmount(gen, i));
						if (dest)
						{
			...
			else
			{
				...
				if (!lut.isDuplicate)
				{
                    ...
                    Texture2D dest = ProcessTexture(gen, i, gen.textureParams[i].additive, gen.textureParams[i].pad, gen.textureParams[i].disableTrimming, false, false, currentTexture, 0, 0, currentTexture.width, currentTexture.height, ref lut, GetPadAmount(gen, i));
					if (dest == null)
					{
						...
						PadTexture(lut.tex, GetPadAmount(gen, i), 0, gen.textureParams[i].pad);
						lut.tex.Apply();
		...
		if (gen.fonts != null)
		{
			for (int i = 0; i < gen.fonts.Length; ++i)
			{
				...
				foreach (var c in fontInfo.chars)
				{
                    ...
                    Texture2D dest = ProcessTexture(gen, i, false, tk2dSpriteCollectionDefinition.Pad.BlackZeroAlpha, false, true, false,
		...
		if (textureList.Length > 0)
		{
            ...
            if (atlasBuilder.Build(gen.freeSizeAtlas) != 0)
			{
        ...
        List<string> oldAtlasTexturePaths = new List<string>(gen.atlasTexturePaths);

		tk2dEditor.Atlas.Data[] atlasData = atlasBuilder.GetAtlasData();
        System.Array.Resize(ref gen.atlasTextures, gen.atlasFormat == tk2dSpriteCollection.AtlasFormat.UnityTexture ? atlasData.Length : 0);
        System.Array.Resize(ref gen.atlasTextureFiles, gen.atlasFormat == tk2dSpriteCollection.AtlasFormat.Png ? atlasData.Length : 0);
        System.Array.Resize(ref gen.atlasTexturePaths, gen.atlasFormat == tk2dSpriteCollection.AtlasFormat.StreamingPNG ? atlasData.Length : 0);
		...
		for (int atlasIndex = 0; atlasIndex < atlasData.Length; ++atlasIndex)
		{
			if (gen.freeSizeAtlas) 
			{
				...
				atlasData[atlasIndex].width = width + GetPadAmount(gen, -1) + 1;
				atlasData[atlasIndex].height = height + GetPadAmount(gen, -1) + 1;
			}
            ...
            else if (gen.atlasFormat == tk2dSpriteCollection.AtlasFormat.Png) 
            {
            ...
            else if (gen.atlasFormat == tk2dSpriteCollection.AtlasFormat.StreamingPNG) 
            {
                texturePath = "Assets/StreamingAssets/tk2dTextures/" + gen.name + atlasIndex + ".png";
            }
            ...
            PNGPostProcess(gen, texturePath);
            ...
            {
                tex = null;
                gen.atlasTexturePaths[atlasIndex] = texturePath;
            }
            ...
            if (gen.forceA8) 
            {
                gen.atlasMaterials[atlasIndex].shader = Shader.Find("tk2d/BlendVertexColorA8");
            }

            if (gen.forceShader != null)
            {
                gen.atlasMaterials[atlasIndex].shader = gen.forceShader;
            }
        ...
        coll.streamingTextures = new string[gen.atlasTexturePaths.Length];
        for (int i = 0; i < gen.atlasTexturePaths.Length; ++i)
        {
            coll.streamingTextures[i] = gen.atlasTexturePaths[i];
        }
        ...
        DeleteUnusedAssets( oldAtlasTexturePaths, gen.atlasTexturePaths );
	...
	static void SetUpTargetTexture(tk2dSpriteCollection gen, Texture2D tex)
	{
        ...
        case tk2dSpriteCollection.TextureCompression.Alpha8: targetFormat = TextureImporterFormat.Alpha8; break;
	...
	static void UpdateFontData(tk2dSpriteCollection gen, float scale, tk2dEditor.Atlas.Data[] packers, List<SpriteLut> spriteLuts, tk2dSpriteCollectionFont font, tk2dEditor.Font.Info fontInfo)
	{
		...
		tk2dEditor.Font.Builder.BuildFont(fontInfo, font.data, 1, font.charPadX, font.dupeCaps, font.flipTextureY, font.gradientTexture, font.gradientCount);
	}
    ...
    static void UpdateVertexCache(tk2dSpriteCollection gen, float scale, tk2dEditor.Atlas.Data[] packers, tk2dSpriteCollectionData coll, List<SpriteLut> spriteLuts)
    {
        for (int i = 0; i < sourceTextures.Length; ++i)
        {
            ...
            coll.spriteDefinitions[i].innerPadding = gen.textureParams[i].disableInnerPadding ? 0 : (int)(gen.innerPadAmount / scale);
            ...
            float atlasWidth = packer.width;
            float atlasHeight = packer.height;

            int xCoordWithoutPadding = 0, yCoordWithoutPadding = 0, widthWithoutPadding = 0, heightWithoutPadding = 0;
    	    if (atlasEntry != null) {
            	xCoordWithoutPadding = atlasEntry.x + padAmount;
            	yCoordWithoutPadding = atlasEntry.y + padAmount;
            	widthWithoutPadding = atlasEntry.w - padAmount * 2;
            	heightWithoutPadding = atlasEntry.h - padAmount * 2;
            }
            int sd_y = packer.height - yCoordWithoutPadding - heightWithoutPadding;

			float uvOffsetX = 0.001f / atlasWidth;
			float uvOffsetY = 0.001f / atlasHeight;

            Vector2 v0 = new Vector2(xCoordWithoutPadding / atlasWidth + uvOffsetX, 1.0f - (sd_y + heightWithoutPadding) / atlasHeight + uvOffsetY);
            Vector2 v1 = new Vector2((xCoordWithoutPadding + widthWithoutPadding) / atlasWidth - uvOffsetX, 1.0f - sd_y / atlasHeight - uvOffsetY);
            ...
            if (thisTexParam.overrideMesh)
            {
                ...
                if (meshFilter == null)
                {
                    CustomDebug.LogError("Unable to find mesh");
            ...
            else
            {
                ...
                Vector3 pos1 = pos0 + new Vector3(scaledWidth, 0, scaledHeight);
				...
				if (_lut != null && _lut.isSplit)
				{
					...
					for (int j = 0; j < spriteLuts.Count; ++j)
					{
						if (spriteLuts[j].source == i)
						{
				            ...
				            sd_y = packer.height - yCoordWithoutPadding - heightWithoutPadding;
				            v0 = new Vector2(xCoordWithoutPadding / atlasWidth + uvOffsetX, 1.0f - (sd_y + heightWithoutPadding) / atlasHeight + uvOffsetY);
				            v1 = new Vector2((xCoordWithoutPadding + widthWithoutPadding) / atlasWidth - uvOffsetX, 1.0f - sd_y / atlasHeight - uvOffsetY);
				...
				else if (thisTexParam.customSpriteGeometry)
				{
					...
					foreach (var island in thisTexParam.geometryIslands)
					{
						...
						for (int x = 0; x < island.points.Length; ++x)
						{
							...
							if (atlasEntry.flipped)
							{
								uv.x = (xCoordWithoutPadding - _lut.ry + texHeight - v.y) / atlasWidth + uvOffsetX;
								uv.y = (yCoordWithoutPadding - _lut.rx + v.x) / atlasHeight + uvOffsetY;
							}
							else
							{
								uv.x = (xCoordWithoutPadding - _lut.rx + v.x) / atlasWidth + uvOffsetX;
								uv.y = (yCoordWithoutPadding - _lut.ry + texHeight - v.y) / atlasHeight + uvOffsetY ;
							}
				...
				else
				{
                    ...
                    Vector3 dpos0 = new Vector3(Mathf.Lerp(pos0.x, pos1.x, x0) - coll.spriteDefinitions[i].innerPadding * scale, 0.0f, Mathf.Lerp(pos0.z, pos1.z, y0) - coll.spriteDefinitions[i].innerPadding * scale);
                    Vector3 dpos1 = new Vector3(Mathf.Lerp(pos0.x, pos1.x, x1) + coll.spriteDefinitions[i].innerPadding * scale, 0.0f, Mathf.Lerp(pos0.z, pos1.z, y1) + coll.spriteDefinitions[i].innerPadding * scale);
				...
				{
					coll.spriteDefinitions[i].positions[j] = positions[j] / scale;
            ...
            coll.spriteDefinitions[i].boundsData[1] = (boundsMax - boundsMin) - new Vector3(2 * coll.spriteDefinitions[i].innerPadding, 2 * coll.spriteDefinitions[i].innerPadding, 0);
			...
			else
			{
				...
				coll.spriteDefinitions[i].untrimmedBoundsData[0] = (boundsMax + boundsMin) / 2.0f / scale;
				coll.spriteDefinitions[i].untrimmedBoundsData[1] = (boundsMax - boundsMin) / scale;
			}
