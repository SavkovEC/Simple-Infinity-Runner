...
public class tk2dSpriteCollectionEditorPopup : EditorWindow, IEditorHost
{
	...
	void UpdateSelection()
	{
		...
		List<SpriteCollectionEditorEntry> tempEntries = new List<SpriteCollectionEditorEntry>();
		foreach (SpriteCollectionEditorEntry entry in entries)
		{
			if (entry.selected)
			{
		...
		tempEntries.Sort((a,b) => a.selectionKey.CompareTo(b.selectionKey));

		selectedEntries = tempEntries;
	}
	...
	public void OnSpriteCollectionSortChanged()
	{
		...
		if (searchFilter.Length > 0)
		{
			foreach (SpriteCollectionEditorEntry entry in entries)
			{
				if (Contains(entry.name, searchFilter))
				{
					tempEntries.Add(entry);
				}
			}
		...
		tempEntries.Sort((a,b) => { int res = a.type.CompareTo(b.type); return res == 0 ? tk2dEditor.Shared.NaturalComparerMethods.Compare(a.name, b.name) : res; });
	...
	string GetEntryTypeString(SpriteCollectionEditorEntry.Type kind)
	{
		...
		CustomDebug.LogError("Unhandled type");
		return "";
	}
