...
public class tk2dSpritePickerPopup  : EditorWindow
{
	...
	void UpdateFilter()
	{
		...
		else
		{
			...
			List<tk2dSpriteDefinition> tempDefinitions = new List<tk2dSpriteDefinition>();

			string s = searchFilter.ToLower();
			if (s != "") 
			{
				foreach (tk2dSpriteDefinition def in spriteCollection.inst.spriteDefinitions)
				{
					if (def.Valid && def.name.ToLower().IndexOf(s) != -1)
					{
			...
			tempDefinitions.Sort(new tk2dEditor.Shared.Tk2dSpriteDefinitionNaturalComparer());
			selectedDefinitions = tempDefinitions;
