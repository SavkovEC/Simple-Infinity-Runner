	...
	void DrawConfigGUI(tk2dCamera _target) {
		...
		if (gameViewFound && gameViewResolutionSet) {
			...
			if (_target.forceResolutionInEditor)
			{
				...
				if (gameViewFound) 
				{
					List<Preset> tempPresetList = new List<Preset>();
					foreach (Preset pr in presets)
					{
						if (pr.MatchAspect(gameViewAspect))
						{
							tempPresetList.Add(pr);
						}
					}

					presetList = tempPresetList;
				}
				else {
				...
				List<string> presetNamesList = new List<string>();
				foreach (Preset pr in presetList)
				{
					presetNamesList.Add(pr.name);
				}

				string[] presetNames = presetNamesList.ToArray();
    ...
    static void DoCreateCameraObject()
	{
		...
		foreach (Camera cam in allCameras) {
			if (cam.cullingMask == -1) {
				CustomDebug.LogError(string.Format("Camera: {0} has Culling Mask set to Everything. This will cause the scene to be drawn multiple times. Did you mean to do this?", cam.name ));
			}
		}
