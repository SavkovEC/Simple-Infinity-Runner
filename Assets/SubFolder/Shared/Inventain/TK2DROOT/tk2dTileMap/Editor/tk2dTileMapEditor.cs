...
public static class tk2dTileMapEditorUtility {
	static tk2dTileMapEditorUtility() {
		...
		if (undoCallback != null) {
			...
			CustomDebug.LogError("tk2d Undo/Redo callback failed. Undo/Redo not supported in this version of Unity.");
		}
#else
...
public class tk2dTileMapEditor : Editor, ITileMapEditorHost
{
	...
	tk2dEditor.BrushRenderer brushRenderer
	{
		...
		set {
			if (value != null) { CustomDebug.LogError("Only alloyed to set to null"); return; }
	...
	tk2dEditor.BrushBuilder guiBrushBuilder
	{
		...
		set {
			if (value != null) { CustomDebug.LogError("Only allowed to set to null"); return; }
	...
	void DrawLoadSaveBrushSection(tk2dTileMapEditorBrush activeBrush)
	{
		...
		if (showSaveSection)
		{
			...
			if (GUILayout.Button("Save"))
			{
				...
				{
					CustomDebug.LogError("Active brush needs a name");
				}
				else
				{
					bool replaced = false;
