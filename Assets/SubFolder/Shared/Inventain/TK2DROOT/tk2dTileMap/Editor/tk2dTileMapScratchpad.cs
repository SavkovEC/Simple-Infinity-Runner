...
public class tk2dScratchpadGUI {
	...
	void UpdateFilteredScratchpads() {
		if (activeScratchpads != null) {
			...
			{
				List<tk2dTileMapScratchpad> tempFilteredScratchpads = new List<tk2dTileMapScratchpad>();
				foreach (tk2dTileMapScratchpad item in activeScratchpads)
				{
					if (Contains(item.name, searchFilter))
					{
						tempFilteredScratchpads.Add(item);
					}
				}

				tempFilteredScratchpads.Sort(new tk2dEditor.Shared.Tk2dTileMapScratchpadNaturalComparer());

				filteredScratchpads = tempFilteredScratchpads;
			}
		}
		else {
